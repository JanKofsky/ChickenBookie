...
After creating an event, the administrator chooses its settlement type in Coop Boss: optional **player-to-player settlement guidance**, where Venmo is not required and players decide whether and how to pay one another after the event, or a **host-maintained pool**. For a host-maintained pool, the host supplies and verifies a Venmo username; bettors supply their own Venmo handle. A bettor can submit several tickets and then use one full-total payment button. All unpaid tickets in that batch share a short payment ID, and the copyable Venmo memo uses `eventname_bettor_paymentid`. The button copies the combined amount, marks the batch as ready for host verification, and opens the host profile in the same tab with best-effort recipient, amount, and note parameters. Returning to Chicken Bookie shows a compact success state with a retry link. Pending Ticket Board entries retain a hoverable and mobile-tappable copy of the exact memo in case the bettor needs to check it again. Submitted tickets remain payment pending—and are excluded from totals, boards, projections, and settlement—until the host matches the payment ID and confirms the whole batch in Coop Boss. Chicken Bookie records these statuses but does not access Venmo transaction data.
