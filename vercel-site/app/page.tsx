...
function Winners({ payload }: { payload: EventPayload }) {
  ...
  return <section className="panel"><h2>Winner's Circle</h2><WinnerCallout payload={payload} /><div className="payment-list">{payments.length === 0 ? <p>No payments needed.</p> : payments.map((payment, idx) => <div className="payment" key={idx}><span><b>{payment.from}</b>{payment.fromVenmo && <code className="venmo-handle">{payment.fromVenmo}</code>} pays <b>{payment.to}</b>{payment.toVenmo && <code className="venmo-handle">{payment.toVenmo}</code>}</span><strong>{money(payment.amount)}</strong></div>)}</div><h3>Ledger</h3><SettlementLedger people={payload.settlement.people} /></section>;
}
