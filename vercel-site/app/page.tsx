...
function Tickets({ payload }: { payload: EventPayload }) {
  ...
  const matchingBets = query ? bets.filter((bet) => [bet.bettor, bet.venmo, bet.paymentId, BET_TYPES[bet.betType], describeBet(bet, chickens, races), money(bet.stake), bet.paymentVerified ? "confirmed counted" : bet.paymentSubmitted ? "pending awaiting host confirmation not counted" : "pending payment not started not counted"].some((value) => normalizeName(String(value)).replace(/^@/, "").includes(query))) : bets;
      ...
      <form id="admin-event-setup" className="grid-form admin-section-target" onSubmit={saveConfig}>
        ...
        {isDropEvent ? <>
          ...
          {races.map((race, idx) => <div className="admin-card" key={race.race}>
            ...
            {!raceStructureLocked && races.length > 1 && <button type="button" className="trash-icon-button" title="Delete race" aria-label={`Delete ${race.name}`} onClick={() => setRaces(races.filter((_, itemIdx) => itemIdx !== idx))}>Delete race</button>}
          </div>)}
          {!raceStructureLocked && <button type="button" className="wide-field add-setup-row" onClick={() => { const race = Math.max(0, ...races.map((item) => item.race)) + 1; setRaces([...races, { race, name: `Race ${race}`, description: "Add race details.", chickenIds: [] }]); }}>+ Add race</button>}
          <h3>Flock notes</h3>
          {chickens.map((chicken, idx) => <div className="admin-card chicken-admin-card" key={chicken.id}>
            ...
            {!raceStructureLocked && chickens.length > 1 && <button type="button" className="trash-icon-button" title="Delete chicken" aria-label={`Delete ${chicken.name}`} onClick={() => { const remaining = chickens.filter((_, itemIdx) => itemIdx !== idx); setChickens(remaining); setRaces(races.map((item) => { if (!item.chickenIds.length) return item; const chickenIds = item.chickenIds.filter((id) => id !== chicken.id); return { ...item, chickenIds: chickenIds.length ? chickenIds : remaining.map((entry) => entry.id) }; })); }}>Delete chicken</button>}
          </div>)}
          {!raceStructureLocked && <button type="button" className="wide-field add-setup-row" onClick={() => { const id = Math.min(0, ...chickens.map((item) => item.id)) - 1; setChickens([...chickens, { id, slot: chickens.length + 1, name: `Chicken ${chickens.length + 1}`, photoUrl: null, bio: "" }]); }}>+ Add chicken</button>}
        </>}
