...
function Tickets({ payload }: { payload: EventPayload }) {
  ...
  return <section className="panel"><h2>Ticket Board</h2>{payload.event.poolMode === "host_managed" && <div className="ticket-count-summary"><strong>{confirmedBets.length} confirmed bet{confirmedBets.length === 1 ? "" : "s"} count</strong>{pendingCount > 0 && <span>{pendingCount} pending · not counted</span>}</div>}<ChickenStatsPanel bets={confirmedBets} chickens={chickens} confirmedOnly={payload.event.poolMode === "host_managed"} />{bets.length === 0 ? <p className="muted">No bets yet.</p> : <div className="ticket-table"><div className="ticket-row ticket-head"><span>Name</span><span>Win condition</span><span>Cluck Bucks</span></div>{bets.map((bet) => <div className={`ticket-row${bet.paymentVerified ? "" : " pending-ticket"}`} key={bet.id}><strong>{bet.bettor}{!bet.paymentVerified && <small className="field-note">{bet.paymentSubmitted ? "NOT COUNTED — awaiting host confirmation" : "NOT COUNTED — payment not started"}</small>}{payload.event.poolMode === "host_managed" && !bet.paymentVerified && <PaymentMemoTip memo={paymentMemo(payload.event.name, bet.bettor, bet.paymentId)} />}</strong><span>{BET_TYPES[bet.betType]} - {describeBet(bet, chickens, races)}</span><b>{!bet.paymentVerified && <small>not counted</small>}{money(bet.stake)}</b></div>)}</div>}</section>;
}
