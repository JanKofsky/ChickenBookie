...
export default function Home() {
  ...
  const gamblers = useMemo(() => new Set(payload?.bets.map((bet) => bet.bettor)).size, [payload]);
...
function CreateEvent({ onCreated }: { onCreated: (payload: EventPayload) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    const data = await response.json();
    if (!response.ok) setMessage(data.error ?? "Could not create event."); else onCreated(data);
  }
  return <section className="panel"><h2>Make a coop</h2><p className="muted">Start with the default flock, or copy an existing event code.</p><form className="grid-form" onSubmit={submit}>
    ...
    <label>Admin code, optional<input type="password" value={adminCode} onChange={(event) => setAdminCode(event.target.value)} /><small>Leave blank if you do not care who can enter winners or delete bets.</small></label>
...
function CoopBoss({ payload, setPayload }: { payload: EventPayload; setPayload: (payload: EventPayload) => void }) {
  ...
  async function save(event: FormEvent) {
    ...
    const data = await response.json();
    if (!response.ok) setMessage(data.error ?? "Could not save winners."); else { setPayload(data); setMessage("Winners saved."); }
  }
  async function removeBet(betId: number) {
    ...
    const data = await response.json();
    if (!response.ok) setMessage(data.error ?? "Could not delete bet."); else { setPayload(data); setMessage("Bet deleted."); }
  }
  return <section className="panel"><h2>Coop Boss</h2><label>Admin code<input type="password" value={adminCode} onChange={(event) => setAdminCode(event.target.value)} /><small>If this event was created with no admin code, leave this blank.</small></label>{payload.bets.length < 2 && <p className="muted">Oh cluck, not enough bets yet. You can save winners, but settlement waits until at least two tickets exist.</p>}<form className="grid-form" onSubmit={save}>{payload.races.map((race) => <label key={race.race}>{race.name}<select value={results[race.race] ?? ""} onChange={(event) => setResults({ ...results, [race.race]: Number(event.target.value) })}><option value="">Pick winner</option>{payload.chickens.map((chicken) => <option key={chicken.id} value={chicken.id}>{chicken.name}</option>)}</select></label>)}<button type="submit">Save winners</button></form>{message && <p className={message.includes("saved") || message.includes("deleted") ? "form-ok" : "form-error"}>{message}</p>}<h3>Delete accidental bet</h3>{payload.bets.length === 0 ? <p className="muted">No accidental bets to delete.</p> : payload.bets.map((bet) => <button className="delete-row" key={bet.id} onClick={() => removeBet(bet.id)}>Delete #{bet.id} - {bet.bettor} - {money(bet.stake)}</button>)}</section>;
}
